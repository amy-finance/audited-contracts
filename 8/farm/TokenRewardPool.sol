...
contract TokenRewardPool is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function transfer(uint256 _poolId, address _sender, address receiver, uint256 _amount)
        external
        nonReentrant
        poolExists(_poolId)
        isAuthSender(_poolId)
    {
        ...
        _withdrawOnly(_poolId, _sender, _amount);
        _stakeOnly(_poolId, receiver, _amount);
    }
        ...
        returns (uint256)
    {
        Pool storage pool = pools[_poolId];
        PoolUser storage poolUser = pool.users[_userAddress];
        uint256 pendingReward = poolUser.stakingAmount.mul(pool.rewardsAccumulatedPerStakingToken.sub(poolUser.rewardsAmountPerStakingTokenPaid).div(DENOMINATOR));
